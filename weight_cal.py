...
def clustering(cluster_id,weights):
    ...
    np.save('ASTclass3weights_SDEC_13_2.npy',weights.detach().numpy())
    
    features, y, adj = load_graph_data('certong_3_m', show_details=False)
    class1=[]
    class2=[]
    class3=[]
    for i in range(len(y)):
        if cluster_id[i]==0:
            class1.append(features[i,4])
        elif cluster_id[i]==1:
            class2.append(features[i,4])
        elif cluster_id[i]==2:
            class3.append(features[i,4])
...
#     print('t',t.shape)
weights = np.load("class3weights_SDEC_13_2.npy", allow_pickle=True)
